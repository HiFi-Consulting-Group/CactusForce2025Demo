...
export default class GranteeHeatmapFilters extends LightningElement {
    ...
    globalMapColumns = [
        { label: 'Opp Name', fieldName: 'RecordURL', sortable: "true", type: 'url', typeAttributes: {label: {fieldName: 'Name'}, target: '_blank'}},
        { label: 'Country', fieldName: 'CountryCode', sortable: "true" },
		{ label: 'Focus Area', fieldName: 'Focus_Area__c', sortable: "true" },
		{ label: 'Grant Investigator', fieldName: 'GrantInvestigator', sortable: "true" },
        { label: 'Amount', fieldName: 'Amount', type: 'currency', sortable: "true"},
        { label: 'Rec Submitted Date', fieldName: 'Rec_Submitted__c', type: 'date', sortable: "true" },
    ];
	...
	renderedCallback() {
		...
		this.template.querySelector('[data-id="global-map-iframe"]').width = window.innerWidth-75;
		this.template.querySelector('[data-id="us-map-iframe"]').width = window.innerWidth-75;
	}
	...
	async loadGranteeData(loadAll) {
		console.log('this.selectedGrantYears',this.selectedGrantYears);
		console.log('this.selectedInvestigators',this.selectedInvestigators);
		...
		let [globalGrantees,usGrantees] = await Promise.all([getGlobalGranteeDetail({filteredGrantYears:grantYearsToFilter,filteredFocusAreas:focusAreasToFilter,filteredInvestigators:investigatorsToFilter}),getUSGranteeDetail({filteredGrantYears:grantYearsToFilter,filteredFocusAreas:focusAreasToFilter,filteredInvestigators:investigatorsToFilter})]);

		console.log('globalGrantees returned: ',globalGrantees);

        this.globalMapOpps = this.updateGlobalGranteeData(JSON.parse(globalGrantees));
		console.log('this.globalMapOpps',this.globalMapOpps);
		this.usMapOpps = this.updateUSGranteeData(JSON.parse(usGrantees));

	}

	async redrawUSMap() {
        var UsMapIframe = this.template.querySelector('[data-id="us-map-iframe"]').contentWindow;
		UsMapIframe.postMessage({name:"redraw"}, this.vfDomain);
	}

	async handleRegionClick(message) {
		console.log('handleRegionClick');
	...
	filterUSDetailData(stateCode) {
		console.log('filterUSDetailData',stateCode);
		console.log('this.usMapOpps',this.usMapOpps);
		...
		this.filteredUsMapOpps = this.doSort(filteredList,this.usSortedBy,this.usSortedDirection);
	}

	filterGlobalDetailData(countryCode) {
		console.log('filterGlobalDetailData',countryCode);
		if (countryCode==='US') {
			this.showUSMap();
		} else {
			this.hideUSMap();
		...
		this.filteredGlobalMapOpps = this.doSort(filteredList,this.globalSortedBy,this.globalSortedDirection);
		console.log('this.filteredGlobalMapOpps',JSON.parse(JSON.stringify(this.filteredGlobalMapOpps)));
	}

	hideUSMap() {
		this.template.querySelector('[data-id="USSection"]').classList.add("hidden-elements");
	}

	showUSMap() {
		console.log('showUSMap');
		this.template.querySelector('[data-id="USSection"]').classList.remove("hidden-elements");
		this.redrawUSMap();
	}
	...
	updateUSGranteeData(granteeData) {
		...
		console.log('granteeData',granteeData);
		for (let opp of granteeData) {
            opp.AccountName = opp.Account.Name;
			opp.RecordURL = oppLinkBase + opp.Id + '/view';
			console.log('opp.Grant_Investigator_User__r',opp.Grant_Investigator_User__r);
			if (opp.Grant_Investigator_User__r && opp.Grant_Investigator_User__r.Name) {
				opp.GrantInvestigator = opp.Grant_Investigator_User__r.Name;
			}
            if (opp.Mapping_Override_Address__c && opp.Mapping_Override_Address__c.stateCode) {
    ...
    updateGlobalGranteeData(granteeData) {
        ...
        for (let opp of granteeData) {
            ...
            if (opp.Mapping_Override_Address__c && opp.Mapping_Override_Address__c.countryCode) {
	...
	async grantYearsChanged(event) {
		...
		var globalMapIframe = this.template.querySelector('[data-id="global-map-iframe"]').contentWindow;
        var UsMapIframe = this.template.querySelector('[data-id="us-map-iframe"]').contentWindow;

		globalMapIframe.postMessage({name:"grantYears",payload:this.selectedGrantYears}, this.vfDomain);
        UsMapIframe.postMessage({name:"grantYears",payload:this.selectedGrantYears}, this.vfDomain);

		await this.loadGranteeData();
		this.resetStartingData();

		// Show filtered data
		this.filteredGlobalMapOpps = this.doSort(this.globalMapOpps,this.globalSortedBy,this.globalSortedDirection);
		this.filteredUsMapOpps = this.doSort(this.usMapOpps,this.usSortedBy,this.usSortedDirection);
    ...
    async focusAreasChanged(event) {
		...
		globalMapIframe.postMessage({name:"focusAreas",payload:this.selectedFocusAreas}, this.vfDomain);
        UsMapIframe.postMessage({name:"focusAreas",payload:this.selectedFocusAreas}, this.vfDomain);
    ...
    async investigatorsChanged(event) {
		...
		globalMapIframe.postMessage({name:"investigators",payload:this.selectedInvestigators}, this.vfDomain);
        UsMapIframe.postMessage({name:"investigators",payload:this.selectedInvestigators}, this.vfDomain);
	...
	handleGlobalSort(event) {
        ...
        this.filteredGlobalMapOpps = this.doSort(parseData,fieldName,sortDirection);	
	}
	...
	handleUSSort(event) {
		...
		this.filteredUsMapOpps = this.doSort(parseData,fieldName,sortDirection);	
	}
